...
// these are very manual tests of basic RawProto stuff
// you should see query for better examples

import RawProto, { decoders } from 'rawproto'
...
test('Submessages', () => {
  ...
  */
  ...
  const s = new RawProto(tree[0].value).readMessage()
...
test('Packed Repeated Fields', () => {
  /*
    6: {5 270 86942}
  */
  const tree = new RawProto([0x32, 0x06, 0x05, 0x8e, 0x02, 0x9e, 0xa7, 0x05]).readMessage()
  expect(tree.length).toEqual(1)
